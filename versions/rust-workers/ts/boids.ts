...
export default class Boids extends THREE.InstancedMesh {
    ...
    randomPositions() {
        for (let i = 0; i < this.boidCount; i++) {
            ...
            pos.position.x = Math.random() * (this.settings.boxSize / 2) * (Math.random() > 0.5 ? -1 : 1);
            pos.position.y = Math.random() * (this.settings.boxSize / 2) * (Math.random() > 0.5 ? -1 : 1);
            pos.position.z = Math.random() * (this.settings.boxSize / 2) * (Math.random() > 0.5 ? -1 : 1);
            pos.updateMatrix();
